...
const Footer = () => {
  return (
    <footer id="footer" className="footer">
      <h1>010-9483-5869</h1>
      {FOOTER_INFO.map((footerInfo, index) => (
        <span key={index}>{footerInfo}</span>
      ))}
      ...
      <div className="footerBottom">
        ...
        <span className="footerBottomText">{FOTTER_TEXT}</span>
      </div>
    </footer>
  );
};

const FOTTER_TEXT = `크로우세븐 주식회사|세븐|12345 강원도 동해시 평원로 하나리움 하나리움
...
const FOOTER_INFO = [
  ...
  '주말 및 공휴일 휴무',
  'XX은행 123-456-7891011 예금주: 스코프컴퍼니(주)',
];
